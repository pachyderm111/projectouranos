...
void loop() {
  ...
  if ((millis() - nowTimeMS) >= loopTime) {
    ...
    //TEMP AND HUMIDITY
    data.concat(String(shellTempC));
    data.concat(",");
    data.concat(String(shellHumid));
    data.concat(",");
